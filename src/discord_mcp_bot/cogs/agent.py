...
def setup(bot):
    ...
    for agent, agent_config in config["agents"].items():
        ...
        @discord.slash_command(
            name=cmd, description="Ask the bot a question", cog=Agent
        )
        async def func(ctx: discord.ApplicationContext, question: str):
            async with get_agent(
                agent_config, config["llms"], config["mcpServers"]
            ) as agent:
                cnt = 0
                first_msg = None
                await ctx.defer()
                        ...
                        if cnt >= 1:
                            if cnt == 1:
                                ...
                                thread_name = await get_thread_name(
                                    agent_config["llm"], config["llms"], question
                                )
